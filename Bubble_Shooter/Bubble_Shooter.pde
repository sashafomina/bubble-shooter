...
void createPointer(Bubble b) {
  if (!_inMotion) { 
    ...
    float endX = startX;
    float endY = startY;
    //for (int i = 0; i < _bubbleField._bubbleGrid.length; i++) {
    //  for (int n = 0; n < _bubbleField._bubbleGrid[i].length; n++) {
    //    if ((endX != _bubbleField._bubbleGrid[i][n]._xcor) && (endY != _bubbleField._bubbleGrid[i][n]._ycor)) {
    while ((startX >= 0) && (startX <= width) && (startY >= 0) && (startY <= height)) {
      endX = startX + cos(angle) * 10;
      endY = startY + sin(angle) * 10;
      for (int i = 0; i < _bubbleField._bubbleGrid.length; i++) {
        for (int n = 0; n < _bubbleField._bubbleGrid[i].length; n++) {
          if ((_bubbleField._bubbleGrid[i][n] != null) && 
          (_bubbleField._bubbleGrid[i][n]._xcor - 20 <= endX) &&
          (endX <= _bubbleField._bubbleGrid[i][n]._xcor + 20) && 
          (endY >= _bubbleField._bubbleGrid[i][n]._ycor + 20) &&
          (endY <= _bubbleField._bubbleGrid[i][n]._ycor - 20)) {
            stroke(0);
            println("poop");
          }
